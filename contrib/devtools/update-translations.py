...
SOURCE_LANG = 'nebulaproject_en.ts'
...
def remove_current_translations():
    ...
    This leaves nebulaproject_en.ts untouched.
    '''
...
def update_build_systems():
    ...
    '''
    filename_lang = [re.match(r'((nebulaproject_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/nebulaproject_locale.qrc', 'w', encoding="utf8") as f:
