...
- [Development guidelines](#development-guidelines)
    - [General NEBULAPROJECT Core](#general-nebulaproject-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
NEBULAPROJECT Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play NBLAs (tNBLA)" on the test network, if you
...
NEBULAPROJECT Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/nebulaproject-project/nebulaproject/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_nebulaproject
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_nebulaproject --log_level=test_suite
$ valgrind -v --leak-check=full src/nebulaprojectd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_nebulaproject.coverage/index.html`.
```

**Sanitizers**
NEBULAPROJECT can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates NBLAs (if wallet is enabled).
...
However, in open source software such as NEBULAPROJECT Core, where everyone uses
...
pay attention to for reviewers of NEBULAPROJECT Core code.

General NEBULAPROJECT Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NEBULAPROJECT Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against NEBULAPROJECT Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/NEBULAPROJECT-Project/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with NEBULAPROJECT's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Nebula-Project/nebula-project.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `nebulaproject-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `nebulaproject-cli` and the GUI debug console use this table to determine how to
