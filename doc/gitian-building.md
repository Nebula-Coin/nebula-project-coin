...
*Setup instructions for a gitian build of NEBULAPROJECT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the NEBULAPROJECT
...
to the NEBULAPROJECT GitHub release page.
...
- [Building NEBULAPROJECT Core](#building-nebulaproject-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/nebulaproject-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/nebulaproject-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/nebulaproject-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/nebulaproject-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/nebulaproject-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/nebulaproject-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/nebulaproject-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@nebulaproject.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/nebulaproject-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/nebulaproject-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/nebulaproject-osx-*-build.assert
```
