...
NEBULAPROJECT Core version *version* is now available from:  <https://github.com/nebulaproject-project/nebulaproject/releases>
...
NEBULAPROJECT v6.0.0 introduces on-chain consensus for masternode lists, which allow for deterministic quorum derivation, implementing Dash's [DIP-0003](https://github.com/dashpay/dips/blob/master/dip-0003.md).
...
A ProTx either creates a 10000-NBLA collateral as tx output, or includes a reference to an unspent 10000-NBLA utxo on chain (and a proof of ownership).
See PR [#2267](https://github.com/Nebula-Project/nebula-project/pull/2267) for more information.
...
* `protx_register`
    ...
    4. "ownerAddress"          (string, required) The NEBULAPROJECT address to use for payee updates and proposal voting.
    ...
    7. "payoutAddress"         (string, required) The NEBULAPROJECT address to use for masternode reward payments.
...
* `protx_register_fund`
    ...
    Arguments:
    1. "collateralAddress"     (string, required) The NEBULAPROJECT address to send the collateral to.
    2. "ipAndPort"             (string, required) IP and port in the form "IP:PORT".
                                  Must be unique on the network. Can be set to 0, which will require a ProUpServTx afterwards.
    3. "ownerAddress"          (string, required) The NEBULAPROJECT address to use for payee updates and proposal voting.
                                  The private key belonging to this address must be known in your wallet, in order to send updates.
                                  The address must not be already registered, and must differ from the collateralAddress
    ...
    5. "votingAddress"         (string, required) The voting key address. The private key does not have to be known by your wallet.
                                  It has to match the private key which is later used when voting on proposals.
                                  If set to an empty string, ownerAddress will be used.
    6. "payoutAddress"         (string, required) The NEBULAPROJECT address to use for masternode reward payments.
...
* `protx_update_registrar`
    ...
    4. "payoutAddress"         (string, required) The NEBULAPROJECT address to use for masternode reward payments.
...
With this rule, a new opcode (`0xd2`) is introduced (see PR [#2275](https://github.com/Nebula-Project/nebula-project/pull/2275)).
