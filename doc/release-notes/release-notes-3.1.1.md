NEBULAPROJECT Core version *3.1.1* is now available from:  <https://github.com/nebulaproject-project/nebulaproject/releases>
...
NEBULAPROJECT Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zNBLA created before block 1050020

The transition to v2 zNBLA and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zNBLA to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zNBLA v1 will now be able to convert that into NBLA and then zNBLA v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zNBLA stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by NEBULAPROJECT stakers.
...
### Show progress percent for znbla reindex operations
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zNBLA code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zNBLA elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for znbla reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zNBLA created before block 1050020. (presstab)
