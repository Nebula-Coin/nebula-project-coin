NEBULAPROJECT Core version *v5.0.0* is now available from:  <https://github.com/nebulaproject-project/nebulaproject/releases>
...
NEBULAPROJECT Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, NEBULAPROJECT Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run NEBULAPROJECT v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving NBLAs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://www.nebulaproject.io .
...
* Top bar showing shield NBLA balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield NBLA balance.
...
See the release notes from [v4.2.0](https://github.com/Nebula-Project/nebula-project/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed NBLA supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in NBLA
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in NBLA
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zNBLAsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield NBLA)
  The `moneysupply` JSON return entry is the total NBLA supply (the sum of all transparent and shield NBLA)
...
| `getaccountaddress` | Returned the current NEBULAPROJECT address associated with an internal account | Account system removal |
| `setaccount` | Associate a NEBULAPROJECT address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given NEBULAPROJECT address | Account system removal |
| `getaddressesbyaccount` | Returned a list of NEBULAPROJECT addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a NEBULAPROJECT address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a NEBULAPROJECT address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zNBLA balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zNBLA mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zNBLA mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zNBLA amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zNBLA to a NBLA address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zNBLA mints to a NBLA address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zNBLAs to see if any mints were added to the chain | Zerocoin deprecation |
| `setznblaseed` | Set the wallet's deterministic zNBLA seed to a specific value | Zerocoin deprecation |
| `getznblaseed` | Return the current deterministic zNBLA seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zNBLA seed | Zerocoin deprecation |
| `dznblastate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdznbla` | Do a search for deterministically generated zNBLA that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_nebulaproject unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake nebulaprojectd and unit tests: fixing missing MacOS framework. (furszy)
