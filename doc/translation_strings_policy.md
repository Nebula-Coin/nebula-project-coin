...
This document provides guidelines for internationalization of the NEBULAPROJECT Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to NEBULAPROJECT network</source>
        <translation>
            <numerusform>%n active connection to NEBULAPROJECT network</numerusform>
            <numerusform>%n active connections to NEBULAPROJECT network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `nebulaproject_en.ts` remains unchanged.
