...
bool CheckBlockSignature(const CBlock& block)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zNBLA or normal UTXO
     *  zNBLA: Each zNBLA has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the coinstake tx.
     */
    CPubKey pubkey;
    bool fzNBLAStake = block.vtx[1]->vin[0].IsZerocoinSpend();
    if (fzNBLAStake) {
        libzerocoin::CoinSpend spend = ZNBLAModule::TxInToZerocoinSpend(block.vtx[1]->vin[0]);
