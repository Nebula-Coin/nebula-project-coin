...
void Shutdown()
{
    ...
    util::ThreadRename("nebulaproject-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", NEBULAPROJECT_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", NEBULAPROJECT_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf("Disable all NEBULAPROJECT specific functionality (Masternodes, Budgeting) (0-1, default: %u)", 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf("Specify masternode configuration file (default: %s)", NEBULAPROJECT_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf("Set external address:port to get to this masternode (example: %s)", "128.127.106.235:1818"));
...
{
    util::ThreadRename("nebulaproject-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that NEBULAPROJECT is running in a usable environment with all
...
#endif
    LogPrintf("NEBULAPROJECT version %s\n", version_string);
}
...
{
    // Make sure only a single NEBULAPROJECT process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", NEBULAPROJECT_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if NEBULAPROJECT is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if NEBULAPROJECT is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //NEBULAPROJECT specific: zerocoin and spork DB's
                ...
                // NEBULAPROJECT: load previous sessions sporks if we have them.
