...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to nebulaprojectd)");
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                        "Usage:  nebulaproject-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                        "or:     nebulaproject-cli [options] -named <command> [name=value]... Send command to " PACKAGE_NAME " (with named arguments)\n"
                        "or:     nebulaproject-cli [options] help                List commands\n"
                        "or:     nebulaproject-cli [options] help <command>      Get help for a command\n";
            strUsage += "\n" + HelpMessageCli();
        }
        ...
        if (!GetAuthCookie(&strRPCUserColonPass)) {
            throw std::runtime_error(strprintf(
                 _("Could not locate RPC credentials. No authentication cookie could be found, and no rpcpassword is set in the configuration file (%s)"),
                    GetConfigFile(gArgs.GetArg("-conf", NEBULAPROJECT_CONF_FILENAME)).string().c_str()));
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject()) {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to nebulaproject-cli command line.";
                        }
                    }
                } else {
                    // Result
