...
// Copyright (c) 2015-2021 The NEBULAPROJECT Core developers
...
#define URI_SCHEME "nebulaproject"
...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZnbla)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZnbla)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZnbla);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZnbla)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZnbla)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZnbla);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter NEBULAPROJECT address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no NEBULAPROJECT: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::NBLA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert nebulaproject:// to nebulaproject:
    //
    //    Cannot handle this later, because nebulaproject:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::NBLA, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", NEBULAPROJECT_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "NEBULAPROJECT (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "NEBULAPROJECT (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "NEBULAPROJECT.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for NEBULAPROJECT*.lnk
...
{
    return GetAutostartDir() / "nebulaproject.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a nebulaproject.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=NEBULAPROJECT (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=NEBULAPROJECT (regtest)\n";
        else
            optionFile << "Name=NEBULAPROJECT\n";
