...
#include "qt/nebulaproject/splash.h"
#include "qt/nebulaproject/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("nebulaproject-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nebulaproject.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nebulaproject.qrc)
...
/** Class encapsulating NEBULAPROJECT Core startup and shutdown.
...
/** Main NEBULAPROJECT application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (NEBULAPROJECTGUI)
...
private:
    ...
    NEBULAPROJECTGUI* window{nullptr};
...
{
    window = new NEBULAPROJECTGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NEBULAPROJECTGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NEBULAPROJECTGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NEBULAPROJECTGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NEBULAPROJECTGUI::DEFAULT_WALLET);
        ...
        // NEBULAPROJECT: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NEBULAPROJECTGUI::handlePaymentRequest);
        connect(window, &NEBULAPROJECTGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. NEBULAPROJECT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nebulaproject_locale);
    Q_INIT_RESOURCE(nebulaproject);
    ...
    /// 6. Determine availability of data directory and parse nebulaproject.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("NEBULAPROJECT Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("NEBULAPROJECT Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // nebulaproject: links repeatedly have their payment requests routed to this process:
