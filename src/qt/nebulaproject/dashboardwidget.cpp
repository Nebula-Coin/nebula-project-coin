...
#include "qt/nebulaproject/dashboardwidget.h"
#include "qt/nebulaproject/forms/ui_dashboardwidget.h"
#include "qt/nebulaproject/sendconfirmdialog.h"
#include "qt/nebulaproject/txrow.h"
...
DashboardWidget::DashboardWidget(NEBULAPROJECTGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareNbla, "square-chart-nbla");
    setCssProperty(ui->labelSquarezNbla, "square-chart-znbla");
    setCssProperty(ui->labelNbla, "text-chart-nbla");
    setCssProperty(ui->labelZnbla, "text-chart-znbla");
    ...
    setCssProperty(ui->labelAmountNbla, "text-stake-nbla-disable");
    setCssProperty(ui->labelAmountZnbla, "text-stake-znbla-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &NEBULAPROJECTGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("NBLA")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isNbla = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZNBLA;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NBLA, zNBLA
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            nbla = (pair.first != 0) ? pair.first / 100000000 : 0;
            znbla = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalNbla += pair.first;
            chartData->totalZnbla += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesNbla.append(nbla);
        chartData->valueszNbla.append(znbla);

        int max = std::max(nbla, znbla);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesNbla);
    set1->append(chartData->valueszNbla);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalNbla > 0 || chartData->totalZnbla > 0) {
        setCssProperty(ui->labelAmountNbla, "text-stake-nbla");
        setCssProperty(ui->labelAmountZnbla, "text-stake-znbla");
    } else {
        setCssProperty(ui->labelAmountNbla, "text-stake-nbla-disable");
        setCssProperty(ui->labelAmountZnbla, "text-stake-znbla-disable");
    }
    forceUpdateStyle({ui->labelAmountNbla, ui->labelAmountZnbla});
    ui->labelAmountNbla->setText(GUIUtil::formatBalance(chartData->totalNbla, nDisplayUnit));
    ui->labelAmountZnbla->setText(GUIUtil::formatBalance(chartData->totalZnbla, nDisplayUnit, true));
