...
OperationResult GovernanceModel::validatePropAmount(CAmount amount) const
{
    if (amount < PROPOSAL_MIN_AMOUNT) { // Future: move constant to a budget interface.
        return {false, strprintf(_("Amount below the minimum of %s NBLA"), FormatMoney(PROPOSAL_MIN_AMOUNT))};
    }

    if (amount > getMaxAvailableBudgetAmount()) {
        return {false, strprintf(_("Amount exceeding the maximum available of %s NBLA"), FormatMoney(getMaxAvailableBudgetAmount()))};
    }
    return {true};
}
