...
#include "qt/nebulaproject/forms/ui_masternodeswidget.h"

#include "qt/nebulaproject/qtutils.h"
#include "qt/nebulaproject/mnrow.h"
#include "qt/nebulaproject/mninfodialog.h"
#include "qt/nebulaproject/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(NEBULAPROJECTGUI *parent) :
    PWidget(parent),
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:1818 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= mnCollateralAmount) {
        inform(tr("Not enough balance to create a masternode, %1 required.")
            .arg(GUIUtil::formatBalance(mnCollateralAmount, BitcoinUnits::NBLA)));
        return;
    }
    showHideOp(true);
