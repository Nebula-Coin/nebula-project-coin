...
PWidget::PWidget(NEBULAPROJECTGUI* _window, QWidget *parent) : QWidget((parent) ? parent : _window), window(_window) { init(); }
PWidget::PWidget(PWidget* parent) : QWidget(parent), window(parent->getWindow()) { init(); }

void PWidget::init()
{
    if (window)
        connect(window, &NEBULAPROJECTGUI::themeChanged, this, &PWidget::onChangeTheme);
}
