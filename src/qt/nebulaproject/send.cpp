...
#include "qt/nebulaproject/send.h"
#include "qt/nebulaproject/forms/ui_send.h"
#include "qt/nebulaproject/addnewcontactdialog.h"
#include "qt/nebulaproject/qtutils.h"
#include "qt/nebulaproject/sendchangeaddressdialog.h"
#include "qt/nebulaproject/optionbutton.h"
#include "qt/nebulaproject/sendconfirmdialog.h"
#include "qt/nebulaproject/guitransactionsutils.h"
...
SendWidget::SendWidget(NEBULAPROJECTGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a NEBULAPROJECT URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-nbla");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onNBLASelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onNBLASelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent NBLAs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield NBLAs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent NBLAs to shield."));
    }
}
...
void SendWidget::onNBLASelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto nblaAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(nblaAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(nblaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(nblaAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
