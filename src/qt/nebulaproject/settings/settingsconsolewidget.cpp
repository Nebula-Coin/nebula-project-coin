...
#include "qt/nebulaproject/settings/forms/ui_settingsconsolewidget.h"
...
#include "qt/nebulaproject/settings/moc_settingsconsolewidget.cpp"
...
SettingsConsoleWidget::SettingsConsoleWidget(NEBULAPROJECTGUI* _window, QWidget *parent) :
...
void SettingsConsoleWidget::clear(bool clearHistory)
{
    ...
    messageInternal(CMD_REPLY, (tr("Welcome to the NEBULAPROJECT RPC console.") + "<br>" +
...
static bool PotentiallyDangerousCommand(const QString& cmd)
{
    ...
    if (cmd.size() >= 13 && cmd.leftRef(11) == "dumpprivkey") {
        // valid NEBULAPROJECT Transparent Address
        std::vector<std::string> args;
    ...
    if (cmd.size() >= 18 && cmd.leftRef(16) == "exportsaplingkey") {
        // valid NEBULAPROJECT Shield Address
        std::vector<std::string> args;
        parseCommandLineSettings(args, cmd.toStdString());
