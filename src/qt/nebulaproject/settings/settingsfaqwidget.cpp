    ...
    clientModel(_model)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableNBLA,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableNBLA,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableNBLA,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("NEBULAPROJECT is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("NEBULAPROJECT utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableNBLAContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received NEBULAPROJECT requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your NEBULAPROJECT wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableNBLA->setText(unspendableNBLAContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of NEBULAPROJECT with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("nebulaproject-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.nebulaproject.io'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in NEBULAPROJECT Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the NEBULAPROJECT network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableNBLA, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableNBLA);});
    ...
    if (parent)
        connect(parent, &NEBULAPROJECTGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
