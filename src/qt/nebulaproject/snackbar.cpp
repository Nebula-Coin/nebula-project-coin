...
#include "qt/nebulaproject/forms/ui_snackbar.h"
#include "qt/nebulaproject/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NEBULAPROJECTGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NEBULAPROJECTGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
