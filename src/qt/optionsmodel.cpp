...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::NBLA);
    ...
    if (settings.contains("nAnonymizeNebulaprojectAmount") || reset)
        gArgs.SoftSetArg("-anonymizenebulaprojectamount", settings.value("nAnonymizeNebulaprojectAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nebulaproject.cpp during shutdown to also remove the window positions
