...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "5. \"address\":     (string, required) NEBULAPROJECT address to send payments to\n"
    ...
    if (!pwallet->CreateBudgetFeeTX(wtx, nHash, keyChange, BUDGET_FEE_TX_OLD)) { // 50 NBLA collateral for proposal
