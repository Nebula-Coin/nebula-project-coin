...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total nebulaproject balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"nebulaprojectaddress\"\n"
            "\nReturn information about the given nebulaproject address.\n"

            "\nArguments:\n"
            "1. \"nebulaprojectaddress\"     (string, required) The nebulaproject address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"nebulaprojectaddress\",    (string) The nebulaproject address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for NEBULAPROJECT cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: NEBULAPROJECT address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are nebulaproject addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) nebulaproject address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"nebulaprojectaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"nebulaprojectaddress\"  (string, required) The nebulaproject address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "nebulaproject-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    { "util",               "validateaddress",        &validateaddress,        true,  {"nebulaprojectaddress"} }, /* uses wallet if enabled */
    { "util",               "verifymessage",          &verifymessage,          true,  {"nebulaprojectaddress","signature","message"} },
