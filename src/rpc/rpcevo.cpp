...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The NEBULAPROJECT address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The NEBULAPROJECT address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The NEBULAPROJECT address to use for masternode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The NEBULAPROJECT address to use for masternode reward payments.\n"
...
// Allows to specify NEBULAPROJECT address or priv key (as strings). In case of NEBULAPROJECT address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid NEBULAPROJECT address %s", strAddress));
    }
    return *keyID;
}
    ...
    if (request.fHelp || request.params.size() < 7 || request.params.size() > 9) {
        throw std::runtime_error(
                ...
                (fSignAndSend ? (
                        ...
                        "\nExamples:\n"
                        + HelpExampleCli("protx_register", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" 0 \"168.192.1.100:1818\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"")
                        ) : (
                        "{                        (json object)\n"
                        ...
                        "}\n"
                        "\nExamples:\n"
                        + HelpExampleCli("protx_register_prepare", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" 0 \"168.192.1.100:1818\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"")
                        )
                )
        );
    }
...
UniValue protx_register_fund(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 6 || request.params.size() > 8) {
        throw std::runtime_error(
                ...
                "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 10000 NBLA\n"
                ...
                "\"txid\"                        (string) The transaction id.\n"
                "\nExamples:\n"
                + HelpExampleCli("protx_register_fund", "\"DKHHBsuU9zfxxxVaqqqQqK4MxZg6vzpf8\" \"168.192.1.100:1818\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\" \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"")
        );
    }
    CheckEvoUpgradeEnforcement();
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        throw std::runtime_error(
                ...
                "\"txid\"                        (string) The transaction id.\n"
                "\nExamples:\n"
                + HelpExampleCli("protx_update_service", "\"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\" \"168.192.1.100:1818\"")
        );
    }
    CheckEvoUpgradeEnforcement();
...
void RegisterEvoRPCCommands(CRPCTable &tableRPC)
{
    if (!Params().IsRegTestNet()) {
        // Disabled before NEBULAPROJECT v6.0
        return;
    }
