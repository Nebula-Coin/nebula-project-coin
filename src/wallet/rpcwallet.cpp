...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"nebulaprojectaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"nebulaprojectaddress\"   (string, required) The nebulaproject address to be associated with a label.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"nebulaprojectaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"nebulaprojectaddress\"  (string, required) The nebulaproject address to send to.\n"
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The nebulaproject staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in NBLA to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The nebulaproject address corresponding to the key that will be able to spend the stake.\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"nebulaprojectaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"nebulaprojectaddress\"  (string, required) The nebulaproject address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"nebulaprojectaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given nebulaprojectaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"nebulaprojectaddress\"  (string, required) The nebulaproject address for transactions.\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. \"keysobject\"   (string, required) A json array of nebulaproject addresses or hex-encoded public keys\n"
            "     [\n"
...
UniValue multisend(const JSONRPCRequest& request)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NBLA address");
