                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s NBLA", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
...
bool CWallet::CreateCoinstakeOuts(const CNblaStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
