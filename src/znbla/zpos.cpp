...
static const CBlockIndex* FindIndexFrom(uint32_t nChecksum, libzerocoin::CoinDenomination denom, int cpHeight)
{
    ...
    // Height limits are ensured by the contextual checks in NewZNblaStake
...
CLegacyZNblaStake* CLegacyZNblaStake::NewZNblaStake(const CTxIn& txin, int nHeight)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend()) {
        LogPrintf("%s: unable to initialize CLegacyZNblaStake from non zc-spend", __func__);
        return nullptr;
    }
    ...
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) ||
        ...
        LogPrint(BCLog::LEGACYZC, "%s : zNBLA stake block: height %d outside range", __func__, nHeight);
        return nullptr;
    }
    ...
    if (_pindexFrom == nullptr) {
        LogPrintf("%s : Failed to find the block index for znbla stake origin", __func__);
        return nullptr;
    }

    // All good
    return new CLegacyZNblaStake(_pindexFrom, _nChecksum, _denom, _hashSerial);
}
