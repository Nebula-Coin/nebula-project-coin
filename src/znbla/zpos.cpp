...
bool CLegacyZNblaStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZNblaStake from non zc-spend", __func__);
    ...
    *this = CLegacyZNblaStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for znbla stake origin", __func__);

    // All good
    return true;
}

CLegacyZNblaStake::CLegacyZNblaStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZNblaStake::GetIndexFrom() const
{
...
CAmount CLegacyZNblaStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZNblaStake::GetUniqueness() const
{
...
bool CLegacyZNblaStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zNBLA stake block: height %d outside range", __func__, nHeight);
