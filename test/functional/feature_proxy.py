...
"""Test nebulaprojectd with different proxy configuration.

Test plan:
- Start nebulaprojectd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on nebulaprojectd side:
...
- Create nebulaprojectds that connect to them
- Manipulate the nebulaprojectds using addnode (onetry) an observe effects
...
class ProxyTest(NebulaprojectTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: nebulaprojectd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
