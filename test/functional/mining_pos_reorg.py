...
class ReorgStakeTest(NebulaprojectTestFramework):
    ...
    def check_money_supply(self, expected_nbla):
        # verify that nodes have the expected NBLA supply
        nbla_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(nbla_supply, [DecimalAmt(expected_nbla)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that NBLA supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check NBLA supply...")
