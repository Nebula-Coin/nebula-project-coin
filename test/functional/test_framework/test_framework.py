...
TMPDIR_PREFIX = "nebulaproject_func_test_"


class NebulaprojectTestFramework():
    """Base class for a nebulaproject test script.

    Individual nebulaproject test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave nebulaprojectds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop nebulaprojectds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing nebulaprojectd/nebulaproject-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use nebulaproject-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: nebulaprojectds were not stopped and may still be running")
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple nebulaprojectds"""
    ...
    def stop_node(self, i):
        """Stop a nebulaprojectd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple nebulaprojectd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as nebulaprojectd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in nebulaproject.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in nebulaproject.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "nebulaprojectd"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run nebulaprojectds:
    ...
    ### NEBULAPROJECT Specific TestFramework ###
    ###################################
...
class ComparisonTestFramework(NebulaprojectTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some nebulaprojectd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "nebulaprojectd"),
                          help="nebulaprojectd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "nebulaprojectd"),
                          help="nebulaprojectd binary to use for reference nodes (if any)")
...
'''
NebulaprojectTestFramework extensions
'''

class NebulaprojectTier2TestFramework(NebulaprojectTestFramework):
